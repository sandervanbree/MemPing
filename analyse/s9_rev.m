% REVISIONS
%
% This is a barely changed version of s12 script
%
%
% SvB
...
zscore_opt = 1;
blcorr_opt = 1;
cue_or_ping_lock = 1; %lock analysis to cue(1) or ping(2)?
...
rev_path = '\\analyse4.psy.gla.ac.uk\project0318\Sander\memflash\memping\rev\data\eeg_data\';
...
% load(['\\analyse4.psy.gla.ac.uk\project0318\Sander\memflash\memping\rev\data\eeg_data\','lda_rev_bl_powval_c',num2str(cat_lvl),'_r',num2str(ref_opt),'_soa',num2str(soa_opt),'_b',num2str(blcorr_opt),'_z',num2str(zscore_opt)]);
 
 % cue or ping lock?
 if cue_or_ping_lock == 1
     load([rev_path,'lda_rev_enc_ret_noping_cuelock_c1_r3_soa3_b1_z1']);
     
     toi = cuestats_toi;
     tvec = curr_win;
     tofilt = 1:numel(tvec)-1; 
     
     np_dat        = squeeze(bin(1,:,tofilt));
     np_dat_s      = squeeze(bin_s(1,:,tofilt,:));
     
     load([rev_path,'lda_rev_enc_ret_ping_cuelock_c1_r3_soa3_b1_z1']);
     p_dat        = squeeze(bin(1,:,tofilt));
     p_dat_s      = squeeze(bin_s(1,:,tofilt,:));
     
 elseif cue_or_ping_lock == 2
     load([rev_path,'lda_rev_enc_ret_noping_pinglock_c1_r3_soa3_b1_z1']);
     ...
     load([rev_path,'lda_rev_enc_ret_ping_pinglock_c1_r3_soa3_b1_z1']);
     p_dat        = squeeze(bin(1,:,tofilt));
     p_dat_s      = squeeze(bin_s(1,:,tofilt,:));
 end
