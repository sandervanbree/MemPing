% Carry out multivariate pattern analysis (linear discriminant analysis)
% using binary and multi-class classification that will serve as the basis
% for most of the subsequent analyses such as:
%
% - Estimating which stim category (bottom, middle, top) that yields the 
% best classification performance in no-ping trials.
%
% - Once that's locked in, test the main hypothesis.
%
% Note: for mid and bottom level classifiers, you'll have to comment out
% binary classification (nclass > 2)
%
% SvB
...
% SAVING
save([save_path,'lda_c',num2str(cat_lvl),'_r',num2str(ref_opt),'_soa',num2str(soa_opt),'_b',num2str(cfg_c.blcorr_opt),'_z',num2str(cfg_c.zscore_opt)],...
